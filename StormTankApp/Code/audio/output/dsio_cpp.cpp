...
static struct DSData
{
  ...
  float vol;
  uint32_t lastpos;                   // last pos we got
  int32_t bufcnt;                    // filled buffer count
  int32_t ltg;                       // last time got
  float mixbuffer[BUFFERLEN/2];    // 32bit stereo mixing buffer
} g_dsound;
...
uint32_t __stdcall dsInit(DSIOCALLBACK *callback, void *parm, void *hwnd)
{
...
int32_t __stdcall dsGetCurSmp()
{
  ...
  int32_t ndiff = gppos - g_dsound.lastpos;
  if (ndiff < 0)
    ndiff += BUFFERLEN;
