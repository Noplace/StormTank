...
int SpectrumAnalyzer::Initialize(ve::Context* context) {
  ve::shape::Shape::Initialize(context);

  ve::shape::Vertex v[LINE_COUNT*8];
  float xinc = 0;
  for (int i=0;i<LINE_COUNT;++i){
    float w = 1;
    v[i*8+0] = ve::shape::Vertex(dx::XMFLOAT3(xinc,0,0), dx::XMFLOAT2(0,0),dxp::XMCOLOR(1,0,0,1),i);
    v[i*8+1] = ve::shape::Vertex(dx::XMFLOAT3(w+xinc,0,0), dx::XMFLOAT2(1,0),dxp::XMCOLOR(1,0,0,1),i);
    v[i*8+2] = ve::shape::Vertex(dx::XMFLOAT3(xinc,50,0), dx::XMFLOAT2(0,1),dxp::XMCOLOR(1,1,0,1),i);
    v[i*8+3] = ve::shape::Vertex(dx::XMFLOAT3(w+xinc,50,0), dx::XMFLOAT2(1,1),dxp::XMCOLOR(1,1,0,1),i);
    v[i*8+4] = ve::shape::Vertex(dx::XMFLOAT3(xinc,50,0), dx::XMFLOAT2(0,0),dxp::XMCOLOR(1,1,0,1),i);
    v[i*8+5] = ve::shape::Vertex(dx::XMFLOAT3(w+xinc,50,0), dx::XMFLOAT2(1,0),dxp::XMCOLOR(1,1,0,1),i);
    v[i*8+6] = ve::shape::Vertex(dx::XMFLOAT3(xinc,100,0), dx::XMFLOAT2(0,1),dxp::XMCOLOR(0,1,0,1),i);
    v[i*8+7] = ve::shape::Vertex(dx::XMFLOAT3(w+xinc,100,0), dx::XMFLOAT2(1,1),dxp::XMCOLOR(0,1,0,1),i);
    xinc += 2.0f;
  }
    
  memset(&vertex_buffer_,0,sizeof(vertex_buffer_));
  vertex_buffer_.description.byte_width = sizeof(ve::shape::Vertex)*LINE_COUNT*8;
...
void SpectrumAnalyzer::AddPCMData256(float* samples, uint32_t channels, double time_ms) {
  ...
  if (time_counter >= 10.0) {
    ...
    XDSP::FFTUnswizzle((XDSP::XMVECTOR*)scratch, real_samples, nBinsLog2);
